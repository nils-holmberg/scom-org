...
mutate(e_index_rel_follow=e_index/follow) |>
mutate(e_index_rel_follow=e_index) |>
...
summarize(
    ...
    #date=first(date)
    m5_e_index=mean(e_index_rel_follow)
) 
#|> 
...
tmp |> pivot_longer(cols=3:7, names_to="measure", values_to="score") |>
  ...
  theme(
    plot.title = element_text(size = rel(5.0)),
    #legend.position = "top",
    #legend.key.size = unit(1, "cm"),
    legend.text = element_text(size=rel(5.0), face = "bold"),
...
mutate(m0_e_index_rel_follow=e_index) |>
group_by(month) |>
summarize(
    m1_sa_val=mean(sa_val),
    m2_sa_int_abs=mean(sa_int_abs),
