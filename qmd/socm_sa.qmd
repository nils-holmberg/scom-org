...
#install.packages(c('stringi'), lib="~/lib/r-cran", dependencies=TRUE)
#install.packages(c('tidyverse','lme4'), lib="~/lib/r-cran", dependencies=TRUE)
if (T) {
# load packages
library("tidyverse")
#library("quarto")
...
tmp |> 
  pivot_longer(cols=3:8, names_to="measure", values_to="score") |>
  ggplot(aes(x=measure, y=score, fill=as.factor(org_type))) +
  geom_boxplot() +
  #scale_fill_viridis(discrete = TRUE) + 
  scale_fill_viridis_d(labels=org_type_labs) + 
  facet_wrap(vars(measure), scales="free", labeller=as_labeller(custom_labeller)) +
  labs(
    title="Measures by Organizational Field",
    subtitle="Post Sentiment and User Engagement Measures",
    caption="",
    x="Measure",
    y="Score",
    fill="Organizational Field"
  ) +
  theme(
...
tmp |> 
  pivot_longer(cols = 2:7, names_to = "measure", values_to = "score") |>
  ggplot(aes(x = as.Date(paste0(date, "-01")), y = score, color = measure)) + 
  geom_point(size = 1.5) + 
  geom_smooth(method = lm, formula = y ~ x, se = FALSE, linewidth = 1.5) + 
  facet_wrap(vars(measure), scales = "free", labeller = as_labeller(custom_labeller)) +
  scale_color_viridis_d() +  # Use viridis for discrete color scales
  labs(
    title = "Measures by Date of Post",
    subtitle = "Post Sentiment and User Engagement Measures",
    caption = "Sentiment is increasing over time, engagement is decreasing",
    x = "Date of Post",
    y = "Score"
  ) +
  theme(
...
## 240926: measures over time, relative valence
...
select(c(19,22,27,14,23,25,11,6)) |>
#mutate(sa_int_rel_wc=sa_int_abs/wc) |>
mutate(sa_val_rel_wc=sa_val/wc) |>
mutate(e_index_rel=e_index/follow) |>
mutate(e_index_rel=e_index) |>
#mutate(date=as.POSIXct(time, origin="1970-01-01", tz="UTC")) |>
mutate(date=as.Date(date)) |>
...
summarize(
    ...
    m3_wc=mean(wc),
    #m4_sa_int_rel_wc=mean(sa_int_rel_wc),
    m4_sa_val_rel_wc=mean(sa_val_rel_wc),
    #date=first(date),
    m5_e_index=mean(e_index_rel),
    m6_follow=mean(follow)
) 
...
custom_labeller <- function(variable, value) {
  ...
  m3_wc="word count", 
  #m4_sa_int_rel_wc="sentiment relative",
  m4_sa_val_rel_wc="sentiment relative",
  m5_e_index="engagement index",
  m6_follow="follower count"
  )
  return(custom_labels[value])
...
tmp = dtp |> as_tibble() |> 
select(c(19,22,27,14,23,24,25,11,6)) |>
mutate(sa_int_rel_wc=sa_int_abs/wc) |>
mutate(e_index_rel=e_index/follow) |>
mutate(e_index_rel=e_index) |>
#mutate(date=as.POSIXct(time, origin="1970-01-01", tz="UTC")) |>
...
summarize(
    ...
    m5_sa_int_sqrt=mean(sa_int),
    
    #date=first(date),
    m6_e_index=mean(e_index_rel),
    m7_follow=mean(follow)
) 
...
custom_labeller <- function(variable, value) {
  ...
  m2_sa_int_abs="sentiment intensity",
  m3_wc="word count", 
  m4_sa_int_rel_wc="intensity relative",
  m5_sa_int_sqrt="intensity transformed",
  m5_e_index="engagement index",
  m6_follow="follower count"
  )
  return(custom_labels[value])
...
tmp |> pivot_longer(cols=2:8, names_to="measure", values_to="score") |>
...
#  mutate(org = str_trim(org)) %>%  # Trim leading/trailing spaces
#  mutate(org = tolower(org)) %>%  # Convert to lowercase for case-insensitive sorting
#  arrange(org)  # Arrange by 'org' alphabetically
  mutate(org = factor(org, levels = sort(unique(org)))) %>%  # Convert 'org' to factor and sort
  arrange(org)  # Arrange by 'org'
...
### duration of post date by org

```{r, purl=T, fig.width=10, fig.height=40}
#| eval: true
#| output: true

dtp |> select(org, time) |> group_by(org) |> summarize(min_dur=min(max(time)-min(time))) |> arrange(min_dur) |> head()
```
...
# Aggregate the data with added count of org_type
aggregated_data <- tmp %>%
  select(starts_with("m1_"), starts_with("m2_"), starts_with("m3_"),
         starts_with("m4_"), starts_with("m5_"), starts_with("m6_"),
         starts_with("m7_"), org_type) %>%
  group_by(org_type) %>%
  summarize(org_count = n(),  # Count the number of occurrences of each org_type
            across(starts_with("m1_"):starts_with("m6_"),
                   list(mean = ~ mean(., na.rm = TRUE),
                        sd = ~ sd(., na.rm = TRUE))),
            m7_first = first(m7_date),
            m7_last = last(m7_date))

# View the aggregated data with org_type count
result_df = aggregated_data |> as_tibble()
cat(simplermarkdown::md_table(result_df))
```

## 240118: sentiment and egagement
